...
Usage:
    ...
    python analyze_trajectory_cellcounts.py --test
    
    # Full run (4000 patches)
    python analyze_trajectory_cellcounts.py
    
    # Custom parameters
    python analyze_trajectory_cellcounts.py --progression CRC-Conventional --model uni2 --n_per_class 1000
"""
...
def compute_pseudotime_for_patches(
    ...
    """
    logger.info(f"⚙️  Computing Diffusion Pseudotime for {len(patch_ids)} patches...")
...
def run_analysis(
    ...
    try:
        ...
        if train_dir.exists():
            ...
            if missing:
                logger.warning(f"   ⚠️ Missing class folders: {missing}")
        ...
        for idx, row in tqdm(trajectory_df.iterrows(), total=len(trajectory_df), desc="Processing patches"):
            ...
            try:
                ...
                if failed_patches <= 5:  # Only log first 5 failures
                    logger.warning(f"   ⚠️ Failed: {patch_id_fmt} in {slide_id_fmt} - {e}")
        ...
        if len(results_df) > 0:
            ...
            print("\n" + "=" * 60)
            print("⚠️  NO PATCHES PROCESSED SUCCESSFULLY")
            print("=" * 60)
            print("\nPlease check:")
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Output CSV path (default: results/<progression>_<model>_cellcounts.csv)"
    )
    parser.add_argument(
        "--test",
        action="store_true",
